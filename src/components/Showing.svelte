<script lang="ts">
    ...
    export let showing: Models.Showing;
...
<div class={$size + " " + isPicked ? "showing picked" : "showing not-picked"}>
    ...
    <div class="poster">
        <img src={cdn + showing.film.images.find((i) => i.mediaType == Models.MediaType.PosterGraphic).url} alt="" />
    </div>
    <div class="info">
        <div class="sessions">
            {#each showing.sessions as session}
                <p class="time info-item">
                    {pad(new Date(session.showtime).getHours(), 2)}:{pad(new Date(session.showtime).getMinutes(), 2)}
                </p>
            {/each}
        </div>
